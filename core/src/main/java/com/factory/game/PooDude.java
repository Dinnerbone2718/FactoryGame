...
public class PooDude {
    ...
    public PooDude(float startWorldX, float startWorldY) {
    ...
    private void loadTexture() {
        texture = new Texture("spritesheets/poo_dude.png");
        ...
        boat_texture = new Texture("spritesheets/poo_dude_raft.png");
    ...
    public void update(float delta, Player player, Player.CollisionChecker cc) {
        if (!player.isOnRaft()) {
            ...
            if (tpx < worldX - Main.TILE_SCALE * 2.5f) {
                dx = -1;
            } else if (tpx > worldX + Main.TILE_SCALE * 2f) {
                dx = 1;
            }

            if (tpy < worldY - Main.TILE_SCALE * 1.5f) {
                dy = -1;
                facingRow = WALK_ROW_DOWN;
            } else if (tpy > worldY + Main.TILE_SCALE * 1f) {
                dy = 1;
