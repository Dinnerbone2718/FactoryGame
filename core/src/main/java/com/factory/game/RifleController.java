...
public class RifleController {
    ...
    public static float DAMAGE = 27f;

    public static final int MAG_SIZE = 5;

    private static final float FIRE_COOLDOWN = 0.35f;
    private static final float RANGE_TILES = 12f;
    ...
    private static final int RIFLE_SHEET_COLS = 2;
    private static final int RIFLE_SHEET_ROWS = 2;
    private static final float RIFLE_GRIP_FRACTION = 0.30f;
    private static final float RIFLE_VERTICAL_OFFSET_TILES = -0.02f;
    private static final float RIFLE_HORIZONTAL_OFFSET_TILES = -0.1f;

    private Texture rifleTexture;
    private TextureRegion[] rifleFrames;

    public RifleController(
        Player player,
        Camera camera,
        WorldManager world,
    ...
    private void loadRifleTexture() {
        rifleTexture = new Texture("spritesheets/rifle.png");
        int fw = rifleTexture.getWidth() / RIFLE_SHEET_COLS;
        int fh = rifleTexture.getHeight() / RIFLE_SHEET_ROWS;
        rifleFrames = new TextureRegion[RIFLE_SHEET_COLS * RIFLE_SHEET_ROWS];
        for (int row = 0; row < RIFLE_SHEET_ROWS; row++) {
            for (int col = 0; col < RIFLE_SHEET_COLS; col++) {
                rifleFrames[row * RIFLE_SHEET_COLS + col] = new TextureRegion(
                    rifleTexture,
                    col * fw,
                    row * fh,
                    fw,
    ...
    public void update(float delta, boolean uiBlocking) {
        ...
        boolean holding = isHoldingRifle();
    ...
    private void spawnBullet() {
        ...
        float muzzleX = originX + dir[0] * Main.TILE_SCALE * 0.35f;
        float muzzleY = originY + dir[1] * Main.TILE_SCALE * 0.35f;
    ...
    private void startReload() {
        ...
        if (!player.getInventory().hasItem(Item.RIFLE_AMMO, 1)) return;
    ...
    private void finishReload(boolean perfect) {
        ...
        if (needed > 0) {
            ...
            if (take > 0) {
                player.getInventory().removeItem(Item.RIFLE_AMMO, take);
                ammoInMag += take;
            }
        }
    ...
    private int countReserveAmmo() {
        ...
        if (!inv.hasItem(Item.RIFLE_AMMO, 1)) return 0;

        int lo = 1;
        int hi = 1;
        while (hi < 100000 && inv.hasItem(Item.RIFLE_AMMO, hi)) {
            lo = hi;
            hi *= 2;
        }
        while (lo < hi) {
            int mid = (lo + hi + 1) / 2;
            if (inv.hasItem(Item.RIFLE_AMMO, mid)) lo = mid;
            else hi = mid - 1;
        }
        return lo;
    }

    public boolean isHoldingRifle() {
        int sel = inventoryUI.getSelectedSlot();
        if (sel < 0) return false;
        ItemStack stack = player.getInventory().getSlot(sel);
        return stack != null && stack.getItem() == Item.RIFLE;
    }
    ...
    public boolean shouldDrawRifleBehindPlayer() {
        return isHoldingRifle() && isPlayerLookingUp();
    }

    private int currentRifleFrameIndex() {
        if (fireCooldownTimer <= 0f) return 0;
        float progress = 1f - (fireCooldownTimer / FIRE_COOLDOWN);
        int idx = (int) (progress * (RIFLE_SHEET_COLS * RIFLE_SHEET_ROWS));
        return MathUtils.clamp(idx, 0, RIFLE_SHEET_COLS * RIFLE_SHEET_ROWS - 1);
    }

    public void drawRifle(SpriteBatch batch) {
        if (!isHoldingRifle() || rifleFrames == null) return;
        ...
        TextureRegion frame = rifleFrames[currentRifleFrameIndex()];
        ...
        float pivotX =
            originX +
            camera.cameraX +
            Main.TILE_SCALE * RIFLE_HORIZONTAL_OFFSET_TILES * (flip ? -1f : 1f);
        float pivotY =
            originY +
            camera.cameraY +
            Main.TILE_SCALE * RIFLE_VERTICAL_OFFSET_TILES;

        float originOffsetX = flip
            ? width * (1f - RIFLE_GRIP_FRACTION)
            : width * RIFLE_GRIP_FRACTION;
