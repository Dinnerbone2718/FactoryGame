...
public class WorldObject {
    ...
    public HarvestResult tryHit(ItemClass toolClass, Item heldItem) {
    ...
    public boolean isHarvestable() {
        return HarvestRegistry.isHarvestable(type);
    }
    ...
    public void draw(Batch batch, Camera cam) {
        ...
        int frameIndex;
        if (
            cfg != null && cfg.mode == ObjectSpriteCache.SelectionMode.ANIMATED
        ) {
            animTimer += delta;
        ...
        if (shakeTimer > 0f) {
            ...
            shakeY =
                (float) Math.cos(t * Math.PI) * SHAKE_MAGNITUDE * decay * 0.5f;
        }
        ...
        float drawW =
            cfg != null
                ? (cfg.drawWidth / 16f) * Main.TILE_SCALE
                : Main.TILE_SCALE;
        float drawH =
            cfg != null
                ? (cfg.drawHeight / 16f) * Main.TILE_SCALE
                : Main.TILE_SCALE;

        TextureRegion region = ObjectSpriteCache.getRegion(type, frameIndex);
        if (region == null) return;

        TextureRegion normalRegion = ObjectSpriteCache.getNormalRegion(
            type,
            frameIndex
        );
