...
public class GoblinoHutManager {
    ...
    private final Map<String, List<GoblinoRecipe>> recipeCache =
        new HashMap<>();
    private final Map<String, List<GoblinoBarterRecipe>> barterCache =
        new HashMap<>();
    ...
    public List<GoblinoRecipe> getRecipes(WorldObject hut) {
        ...
        return recipeCache.computeIfAbsent(key, k ->
            generateRecipes(hut.getX(), hut.getY())
        );
    }

    public List<GoblinoBarterRecipe> getBarterRecipes(WorldObject hut) {
        String key = hut.getX() + "," + hut.getY();
        return barterCache.computeIfAbsent(key, k ->
            generateBarterRecipes(hut.getX(), hut.getY())
        );
    }

    public GoblinoMood getMood(WorldObject hut) {
        String key = hut.getX() + "," + hut.getY();
        GoblinoMood base = moodCache.computeIfAbsent(key, k ->
            generateMood(hut.getX(), hut.getY())
        );
        ...
        int newOrdinal = Math.max(
            0,
            Math.min(moods.length - 1, base.ordinal() - steps)
        );
    ...
    public NegotiationResult attemptNegotiation(
        WorldObject hut,
        ...
        int minAcceptable = Math.max(
            1,
            Math.round(effectiveCost * currentMood.negotiationFloor)
        );

        if (
            offeredCoins >= minAcceptable &&
            inventory.hasItem(Item.GOBLINO_COIN, offeredCoins)
        ) {
    ...
    public Map<String, Integer> getHappinessSnapshot() {
        return new HashMap<>(happinessMap);
    }

    public void restoreHappiness(Map<String, Integer> data) {
        happinessMap.clear();
        if (data != null) happinessMap.putAll(data);
    }
    ...
    private List<GoblinoRecipe> generateRecipes(int hx, int hy) {
        long seed =
            worldSeed ^ ((long) hx * 0xDEAD_BEEFL) ^ ((long) hy * 0xCAFE_BABEL);
        ...
        return new ArrayList<>(
            pool.subList(0, Math.min(tradeCount, pool.size()))
        );
    }

    private List<GoblinoBarterRecipe> generateBarterRecipes(int hx, int hy) {
        long seed =
            worldSeed ^ ((long) hx * 0xA1B2_C3D4L) ^ ((long) hy * 0xE5F6_7890L);
        Random rng = new Random(seed);
        int tradeCount = MIN_TRADES + rng.nextInt(MAX_TRADES - MIN_TRADES + 1);

        List<GoblinoBarterRecipe> pool =
            CraftingManager.getAllGoblinoBarterRecipes();
        Collections.shuffle(pool, rng);
        return new ArrayList<>(
            pool.subList(0, Math.min(tradeCount, pool.size()))
        );
    ...
    private GoblinoMood generateMood(int hx, int hy) {
        long seed =
            worldSeed ^ ((long) hx * 0xFEED_FACEL) ^ ((long) hy * 0xBEEF_CAFEL);
        GoblinoMood[] moods = GoblinoMood.values();
