...
public final class PlanterManager {
    ...
    public Inventory getOrCreatePlanterInventory(PlacedObject planter) {
        return planterInventories.computeIfAbsent(keyOf(planter), k ->
            new Inventory(PLANTER_INV_SLOTS)
        );
    }
    ...
    private void tickPlanter(
        ...
        LiquidTank waterTank = (tanks != null && tanks.length > 0)
            ? tanks[0]
            : null;
        LiquidTank fertTank = (tanks != null && tanks.length > 1)
            ? tanks[1]
            : null;
        ...
        float waterGot = 0f;
        if (waterTank != null) {
            waterGot = waterTank.withdraw(
                Math.min(waterNeeded, waterTank.getAmount())
            );
        }
        ...
        float fertGot = 0f;
        if (fertTank != null) {
            fertGot = fertTank.withdraw(
                Math.min(fertNeeded, fertTank.getAmount())
            );
        }
        boolean hasFertilizer = fertGot >= fertNeeded * 0.01f;

        float advance =
            delta * (hasFertilizer ? PlanterRecipe.FERTILIZER_SPEED_MULT : 1f);
        state.growTimer += advance;

        float duration = state.recipe.getGrowDuration();
        state.growFrame = Math.min(
            8,
            (int) ((state.growTimer / duration) * 9f)
        );
        ...
        for (int fx = px; fx < px + tw; fx++) {
            for (int fy = py; fy < py + th; fy++) {
                for (int[] d : DIRS) {
                    ...
                    if (
                        pipe == null || pipe.type != PlacedObject.Type.ITEM_PIPE
                    ) continue;
        ...
        for (int fx = px; fx < px + tw; fx++) {
            for (int fy = py; fy < py + th; fy++) {
                for (int[] d : DIRS) {
                    ...
                    if (
                        inv != null &&
                        inv.addItem(drop.getItem(), drop.getQuantity())
                    ) {
                        return;
                    }
                }
    ...
    public Map<String, float[]> getGrowthSnapshot() {
        Map<String, float[]> out = new HashMap<>();
        for (Map.Entry<String, PlanterState> e : states.entrySet()) {
            PlanterState s = e.getValue();
            if (s.plantedSeed == null) continue;
            out.put(
                e.getKey(),
                new float[] {
                    s.plantedSeed.ordinal(),
                    s.growTimer,
                    s.growFrame,
                }
            );
    ...
    public void restoreGrowthSnapshot(Map<String, float[]> data) {
        states.clear();
        if (data == null) return;
        ...
        for (Map.Entry<String, float[]> e : data.entrySet()) {
            float[] v = e.getValue();
            int ord = (int) v[0];
            if (ord < 0 || ord >= items.length) continue;
            Item seed = items[ord];
            PlanterRecipe recipe = CraftingManager.getPlanterRecipeFor(seed);
            if (recipe == null) continue;
            PlanterState state = new PlanterState();
            state.plantedSeed = seed;
            state.recipe = recipe;
            state.growTimer = v[1];
            state.growFrame = (int) v[2];
            states.put(e.getKey(), state);
        }
    }

    public Map<String, Inventory> getPlanterInventoriesSnapshot() {
        return new HashMap<>(planterInventories);
    }

    public void restorePlanterInventories(Map<String, Inventory> data) {
        planterInventories.clear();
        if (data != null) planterInventories.putAll(data);
    }
