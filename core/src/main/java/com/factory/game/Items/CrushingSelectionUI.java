...
public class CrushingSelectionUI {
    ...
    private List<CrushingRecipe> currentRecipes;

    public CrushingSelectionUI(Inventory inventory) {
    ...
    public void open() {
        currentRecipes = CraftingManager.getAvailableCrushRecipes(inventory);
    ...
    public CrushingRecipe pollChosen() {
        CrushingRecipe r = chosenRecipe;
        chosenRecipe     = null;
        return r;
    }
    ...
    public void render(SpriteBatch batch) {
        ...
        font.draw(batch, "What to Crush?", panelX + 14, panelY + panelH - 12);

        for (int i = 0; i < currentRecipes.size(); i++) {
            CrushingRecipe recipe = currentRecipes.get(i);
            ...
            font.setColor(hov ? new Color(1f, 0.92f, 0.35f, 1f) : Color.WHITE);
            font.draw(batch, item.getDisplayName(), labelX, labelY);

            font.getData().setScale(1.05f);
            font.setColor(0.70f, 0.58f, 0.40f, 1f);
            font.draw(batch, "x" + inventory.countItem(item) + " owned", labelX, labelY - 20);
        }

        if (hoveredIndex >= 0) {
            CrushingRecipe recipe = currentRecipes.get(hoveredIndex);
            int infoH = TITLE_H + 32 + 16;
            infoPanelY = panelY + panelH - infoH;

            font.getData().setScale(1.3f);
            font.setColor(1f, 0.82f, 0.50f, 1f);
            font.draw(batch, "Output", infoPanelX + 10, infoPanelY + infoH - 12);

            Item out = recipe.getOutputItem();
            int  qty = recipe.getOutputQuantity();
            int  rowY = infoPanelY + infoH - TITLE_H - 10;

            TextureRegion tex = ItemTextureCache.getTexture(out);
            if (tex != null)
                batch.draw(tex, infoPanelX + 10, rowY - 22, 24, 24);

            font.getData().setScale(1.05f);
            font.setColor(Color.WHITE);
            font.draw(batch, out.getDisplayName() + "  x" + qty, infoPanelX + 40, rowY);

        }

        font.getData().setScale(1.1f);
        font.setColor(0.50f, 0.40f, 0.28f, 0.9f);
    ...
    private void drawPanel(int x, int y, int w, int h) {
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.setColor(0.10f, 0.07f, 0.05f, 0.97f); 
        ...
        shapeRenderer.end();
        shapeRenderer.begin(ShapeRenderer.ShapeType.Line);
        shapeRenderer.setColor(0.38f, 0.24f, 0.14f, 1f);
